...
public class EnemyPathingCubic : EnemyPathing
{
    ...
    private void CubicMove()
    {
        ...
        if (reverse)
        {
            ...
            {
                count += movementThisFrame / 100;
                Vector3 p0 = Vector3.Lerp(waypoints[3].position, waypoints[2].position, count);
                Vector3 p1 = Vector3.Lerp(waypoints[2].position, waypoints[1].position, count);
                Vector3 p2 = Vector3.Lerp(waypoints[1].position, waypoints[0].position, count);
                Vector3 p3 = Vector3.Lerp(p0, p1, count);
                Vector3 p4 = Vector3.Lerp(p1, p2, count);
                transform.position = Vector3.Lerp(p3, p4, count);
                Vector3 v = p3 - p4;
                dirOfTravel = Mathf.Atan2(v.x, v.y) * Mathf.Rad2Deg;
            }
            else
            {
                count = 0;
